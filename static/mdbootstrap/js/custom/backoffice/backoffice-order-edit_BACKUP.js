...
function loadAndUpdateMainList(newProduct, quantityAdded) {
    if (!newProduct) {
        const parent_el = $("#cart-main-list").find(`[data-product-id=${cart[0]['id']}]`); // this is the <tr> with the data-product-id attribute of the respective product
        ...
        $(parent_el).find(".product-quantity-field").val(newQuantity);
        ...
        while(cart.length>0) {
            addItemsToMainList(cart[0]);
            cart.shift() // once the element has been added, remove it from the array so that it won't be double added the next time we run this loop
        }
    }
}

function addItemsToMainList(data) {
    ...
    $(template).find(".product-bottom-price").html(data["bottomPrice"]);
    $(template).find(".product-price").html(data["price"]);
    $(template).find(".product-full-price").html(`${data["price"]*data["cart_quantity"]}`);
...
function updateTotalPrice(product_id) {
    ...
    const discount_price = $(el).find('.discount-price').html();

    if(!parseInt(discount_price)) { // means there is no discount price shown
        new_price = parseFloat($(el).find('.product-price').html());
    }
    else {
    ...
    $(el).find('.product-full-price').html(new_price);
}

function alterCartCallback(res) {
    let resData = JSON.parse(res);

    loadAndUpdateMainList(resData.newProduct, resData.quantity); // the loadAndUpdateMainList() function will act differently depending on whether the product added is a new product or not. I fi IS a new product, a new Zeile will be added - if not, the existing Zeile will be increased
    
    if(resData['quantity']==0) {
        let rowToDelete = $(`tr[data-product-id="${resData['productId']}"]`); // the BackOffice-table contains each product-list item in a <tr>
        rowToDelete.remove();
    }
    else {
...
function addProductCallback(res) { // 1. processes the products-popup search bar
    ...
    console.log('products XXX products XXX products XXX products XXX products XXX ');
    console.log(data);
...
function sendRequest(endpoint, data) {
    ...
    if (endpoint == select_customer_endpoint) {
        ...
        if(data.dataset.action == 'add' || data.dataset.action == 'addCF') {
            ...
            console.log($(data));
            /////////////////////////////
            const parent = $(data).closest('.quantity-field');
            const minus_button = $(parent).find('.disabled');
